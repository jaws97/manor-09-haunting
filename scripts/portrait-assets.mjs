#!/usr/bin/env node
/**
 * Derives everything the show needs from each portrait in public/portraits/NN.webp:
 *   NN-sm.webp  thumbnail for the gate page wall and the review page
 *   NN-bg.webp  pre-blurred ambient backdrop for that resident's room (blurring
 *               here keeps a heavy CSS blur off the projector laptop's GPU)
 *   src/data/portrait-colors.json  one accent colour per resident
 * Re-run after adding or replacing a portrait:  node scripts/portrait-assets.mjs
 */
import { readdirSync, writeFileSync } from "node:fs";
import sharp from "sharp";

const DIR = "public/portraits";
const portraits = readdirSync(DIR).filter((f) => /^\d\d\.webp$/.test(f)).sort();
...
for (const file of portraits) {
  ...
  await sharp(`${DIR}/${file}`)
    ...
    .blur(18)
    .modulate({ brightness: 0.8, saturation: 1.6 })
    .webp({ quality: 70 })
...
writeFileSync("src/data/portrait-colors.json", JSON.stringify(colors, null, 2) + "\n");
console.log(`\n${portraits.length} portraits processed`);
