...
const { rows } = await db.query("select table_name from information_schema.tables where table_name like 'm09_%' order by 1");
console.log("tables:", rows.map((r) => r.table_name).join(", "));
const fns = await db.query("select proname from pg_proc where proname like 'm09_%' order by 1");
console.log("functions:", fns.rows.map((r) => r.proname).join(", "));
if (process.argv.includes("--wipe")) {
  await db.query("truncate m09_tickets, m09_wishes, m09_photos; update m09_show set state = '{}'::jsonb, rev = rev + 1 where id = 'main'");
  console.log("wiped: invitations, whispers, photos, show state");
}
await db.end();
...
if (buckets.some((b) => b.name === "m09-photos")) console.log("bucket: m09-photos already exists");
else {
  const made = await sb.storage.createBucket("m09-photos", { public: false, fileSizeLimit: 1_500_000 });
  if (made.error) throw made.error;
  console.log("bucket: m09-photos created (private)");
}
